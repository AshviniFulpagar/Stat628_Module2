...
Bodyfat = function(x){
  # when the user give us all 7 varaibles, we use model 2 to predict
  if (sum(is.na(x))==0){
    fat = -11.87331-0.04943*x[1]+0.73379*x[2]+0.04650*x[3]-
      0.03281*x[4]+0.34329*x[5]-0.26062*x[6]-1.46973*x[7]
  }else if(sum(is.na(x[1:4]))==0){  # when we don't have all 7 variables but have the most important 4 varisbles, use model 1
    fat = -27.76-0.09248*x[1]+0.837*x[2]-0.0009792*x[3]-0.2008*x[4]
  }
