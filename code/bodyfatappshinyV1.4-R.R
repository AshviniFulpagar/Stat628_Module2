...
Bodyfat = function(x){
  ...
  if (sum(is.na(x[1:4]))>0){
    ...
    value = "ERROR: There is something wrong with your input, please recheck it!"
    suggest = "ERROR: There is something wrong with your input, please recheck it!"
  }else if(fat<2 | fat>45){
    value = paste0("Warning: Your body fat percentage is ",fat,
                   ". But there may be something wrong in your input because this body fat is unusual.")
...
ui = fluidPage(
  ...
  sidebarPanel(
      #width=10,
      h6("Note: Items with * are required. Fill all of the iteams to get a more acuurate result."),
      hr(),
      ...
      submitButton("Update your body fat"), #click the button to predict
    ),
